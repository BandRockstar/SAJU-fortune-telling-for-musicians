...
# 1️⃣ 페이지 설정 및 UI 디자인
st.set_page_config(page_title="음악인을 위한 사주통변 Ver 1.4", layout="centered")
st.markdown("""
    <style>
...
st.markdown("<div class='main-title'><h1>🎸 음악인을 위한 사주통변 Ver 1.4</h1></div>", unsafe_allow_html=True)
...
analysis_db = {
    '비겁': {
        ...
        'instrument': """
            귀하에게 가장 어울리는 파트는 **보컬**과 **기타**입니다. 비겁의 강한 주체성은 무대 중앙에서 군중을 압도하는 에너지를 필요로 합니다. 보컬로서는 가창의 기교보다는 목소리 자체의 힘과 진정성으로 청중의 심장을 관통하는 스타일이 적합하며, 직선적이고 파워풀한 벨팅 창법이 기질과 잘 맞습니다. 기타를 잡는다면 섬세한 백킹보다는 화려하고 공격적인 리드 기타가 운명적입니다. 본인만의 독보적인 톤을 구축하여 전체 사운드의 중심을 뚫고 나오는 선명한 존재감을 드러낼 때, 비로소 예술적 갈증이 해소될 것입니다. 밴드 내에서도 다른 악기에 묻히기보다 사운드의 주인공으로서 기능할 때 가장 큰 음악적 성취를 거둘 수 있습니다.
        """
    },
    '식상': {
        ...
        'mus': "창작의 마르지 않는 샘물을 가진 아티스트로서, 멜로디와 가사를 직조하는 감각이 본능적으로 발달해 있습니다. 화려한 연주 테크닉은 물론, 청중의 심리를 파고드는 감성적인 톤 메이킹에 천부적인 소질을 보입니다. 즉흥적인 영감을 소리로 표현하는 데 능합니다.",
    ...
    '재성': {
        'gen': "본인은 현실을 꿰뚫어 보는 안목과 공간을 장악하는 능력이 남다른 명식으로, 매사에 실리적이고 결과 중심적인 사고를 통해 목표를 성취하는 유능한 성품의 소유자입니다. 환경을 본인에게 유리하게 조율하는 힘을 상징하며, 본질을 빠르게 파악하여 가장 효율적인 해결책을 찾아냅니다.",
        ...
        'instrument': """
            음악의 뼈대를 형성하고 전체 흐름을 제어하는 귀하에게는 **드럼**과 **베이스**가 운명적인 악기입니다. 재성의 현실 장악력은 무대 뒤편에서 전체 앙상블의 박자와 리듬을 완벽하게 통제할 때 가장 빛납니다. 드럼은 곡의 구조를 결정짓는 설계도와 같으며, 베이스는 화성과 리듬을 연결하는 가교 역할을 하기에 본인의 관리자적 기질과 공명합니다. 화려하게 전면에 나서기보다 전체 사운드가 비지 않도록 채워주는 견고한 플레이에서 깊은 만족감을 느끼며, 동료 뮤지션들이 마음 놓고 연주할 수 있는 가장 든든한 기반이 될 것입니다. 보컬을 한다면 안정적인 호흡을 바탕으로 곡의 서사를 이끄는 스타일이 적합합니다.
        """
    },
    '관성': {
        'gen': "본인은 명예와 규율을 중요하게 여기는 강직한 성품과 완벽주의를 소유한 명식입니다. 자신에게 엄격한 기준을 적용하는 만큼 타인에게도 정직과 신뢰를 요구하며, 조직 내에서 올바른 질서를 유지하는 데 중추적인 역할을 수행합니다.",
        ...
        'instrument': """
            정밀함과 규율을 중시하는 귀하에게는 **키보드**와 **베이스**가 가장 적합합니다. 키보드는 화성학적 기초가 탄탄해야 하며 정교한 터치가 요구되는 악기이기에 귀하의 완벽주의적 기질을 투영하기 좋습니다. 또한 베이스 연주 시에도 단 한 박의 오차도 허용하지 않는 메트로놈 같은 정확성을 보여주어 밴드의 음악적 품격을 높입니다. 보컬 스타일은 정석적인 발성과 정갈한 창법을 선호하며, 가사의 전달력을 극대화하는 클래식한 창법에서 강점을 보입니다. 귀하의 소리는 청중에게 깊은 신뢰감을 주며, 정교하게 세공된 사운드의 정수를 보여줄 것입니다. 정해진 약속(Score)을 완벽하게 이행하는 모습에서 아티스트로서의 독보적 권위가 세워질 것입니다.
        """
    },
    '인성': {
        'gen': "본인은 깊은 통찰력과 지혜로운 사유를 바탕으로 현상의 본질을 탐구하는 지혜로운 명식입니다. 지식의 습득 능력이 뛰어나고 이를 본인만의 철학으로 재해석하여 세상에 내놓는 정신적인 창조 능력이 탁월합니다.",
        'mus': "음악 속에 심오한 철학적 메시지와 신비로운 무의식의 영역을 담아내는 능력이 탁월합니다. 단순히 청각적 즐거움을 넘어 하나의 완벽한 세계관을 창조하는 과정을 즐기며, 보이지 않는 정서를 선율로 치환합니다.",
        'pos_title': "🎼 수석 편곡가 & 키보디스트",
        'pos_desc': "곡의 내러티브를 설계하고 세련된 화성으로 사운드를 풍성하게 만드는 분야에서 독보적입니다. 보이지 않는 감정을 선율로 치환하는 능력을 갖추고 있습니다.",
        'instrument': """
            깊은 사유와 영적인 울림을 추구하는 귀하에게는 **기타(어쿠스틱)**와 **키보드(피아노 계열)**가 가장 잘 어울립니다. 화성적으로 풍부한 울림을 가진 악기를 통해 본인의 내면세계를 확장하며, 여운이 길게 남는 선율을 직조할 때 예술적 영감이 극대화됩니다. 보컬 스타일은 가사의 이면을 읽어내어 관객에게 이야기를 들려주는 스토리텔링형 보컬이 적합하며, 몽환적인 분위기를 자아내는 창법이 기질과 공명합니다. 기술적인 화려함보다는 소리 한 점에 담긴 감정의 밀도를 중시하므로, 청중의 무의식을 건드리는 지혜로운 선율을 구사하게 될 것입니다. 귀하의 소리는 단순한 음악을 넘어 영혼을 치유하는 특별한 힘을 발휘하게 됩니다.
        """
    }
}
...
if submitted:
    ...
    ba_zi = [lunar.getYearInGanZhi(), lunar.getMonthInGanZhi(), lunar.getDayInGanZhi(), "辛묘"]
    d_gan = lunar.getDayGan()
    
    # 분석
    max_oh = get_ohaeng_max(ba_zi)
    origin_data = analysis_db[get_sibsung_name(d_gan, max_oh)]
    ...
    <div class='target-year-card'>
        ...
        <div class='content-text'>
            ...
            이 시기에는 본인이 구축해온 음악적 세계관이 외부의 기운과 조응하며 새로운 변화를 맞이하게 됩니다. 
            특히 {t_sib}의 특성에 맞춰 창작의 방향성을 설정한다면 보다 효율적인 예술적 성취를 거둘 수 있을 것입니다.
        </div>
    </div>
    """, unsafe_allow_html=True)
