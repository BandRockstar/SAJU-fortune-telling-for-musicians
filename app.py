...
if st.button("🎭 심층 이원 통변 리포트 생성"):
    if name:
        ...
        if target_year_zi in my_samjae_zis:
            st.error(f"{target_year}년은 삼재 기간입니다.")
            samjae_idx = my_samjae_zis.index(target_year_zi)
            c1, c2, c3 = st.columns(3)
            with c1: st.write("들삼재" if samjae_idx == 0 else " ")
            with c2: st.write("눌삼재" if samjae_idx == 1 else " ")
            with c3: st.write("날삼재" if samjae_idx == 2 else " ")
        else:
            st.success(f"{target_year}년은 삼재 기간이 아닙니다.")
        # ------------------------
