...
with st.form("saju_form"):
    ...
    # 1시간 단위 선택 리스트 (00시 ~ 23시)
    t_options = ["시간 모름"] + [f"{i:02d}시" for i in range(24)]
    u_t = st.selectbox("태어난 시간 (1시간 단위)", t_options)
...
if submitted:
    if not u_name:
        ...
        else: cal.setSolarDate(u_y, m=u_m, d=u_d)
        
        ganji = cal.getChineseGapJaString().split() # 연, 월, 일
        
        # 1시간 단위를 명리 시주(2시간 구간)로 매칭하는 로직
        ...
        <div class="result-card">
            <div class="res-title">📜 {u_name}님의 전 생애 분석</div>
            <div class="res-body">
                <span class="res-sub">● 타고난 천명</span>
                {ganji[2]} 기운을 중심으로 한 당신의 사주는 독창적인 감각과 실천력을 겸비하고 있습니다. 
                <br><br>
                <span class="res-sub">● 2080년까지의 흐름</span>
                2026년부터 운의 흐름이 크게 바뀌며, 그동안 준비해온 일들이 사회적인 결실로 이어집니다. 
                2040년대와 50년대에는 리더로서 명예를 얻고, 2080년까지 안정적인 기운 속에서 평온한 삶을 영위할 운세입니다.
            </div>
        </div>
        """, unsafe_allow_html=True)
        st.balloons()
