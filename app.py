...
if st.button("🎭 심층 이원 통변 리포트 생성"):
    if name:
        ...
        if target_year_zi in my_samjae_zis:
            ...
            st.error(f"🚫 **삼재: {target_year}년은 귀하의 삼재 기간({current_status})입니다.**\n\n삼재는 9년 주기로 돌아와 3년 동안 머무는 에너지의 정체기를 의미합니다. 현재 귀하는 기운이 내면으로 응축되고 외부의 변화에 민감하게 반응하는 시기에 놓여 있습니다. 이 시기에는 새로운 확장보다는 내실을 기하고, 대인 관계에서의 구설이나 예기치 못한 사고를 미연에 방지하기 위해 평소보다 신중한 태도를 유지하는 것이 좋습니다. 특히 감정적인 결단보다는 객관적인 데이터와 주변의 조언을 수용하며 자중자애하는 자세가 액운을 복으로 바꾸는 열쇠가 될 것입니다.")
        else:
            st.success(f"✅ **삼재: {target_year}년은 삼재에 해당하지 않습니다.**\n\n축하드립니다. 현재 귀하는 삼재의 영향권에서 완전히 벗어나 기운이 비교적 맑고 순탄하게 흐르는 운세의 흐름 속에 있습니다. 무거운 기운이 물러가고 본연의 역량을 온전히 발휘할 수 있는 환경이 조성되는 시기이므로, 그동안 계획해왔던 일들을 적극적으로 추진하거나 새로운 도전을 시작하기에 매우 적합한 타이밍입니다. 자신감을 가지고 활동하신다면 길운을 더욱 길게 유지할 수 있을 것입니다.")
        ...
        if my_element == "木": part2 += "오상 중 인(仁)을 상징하는 나무처럼 위로 뻗어 나가려는 향상심과 창의적인 기획력이 돋보입니다. 타인을 배려하는 어진 마음씨와 생동감 넘치는 생명력을 지녔으나, 때로는 굽히지 않는 고집으로 인해 갈등을 빚을 수 있습니다. 늘 새로운 가치를 창조하는 일에서 영혼의 만족감을 얻는 유형입니다."
        elif my_element == "火": part2 += "오상 중 예(禮)를 상징하는 불꽃처럼 열정적이고 화려하며, 자신을 세상에 드러내어 소통하려는 표현력이 대단히 강력합니다. 감수성이 풍부하고 명확한 것을 선호하며 주변을 밝게 만드는 긍정적인 에너지를 전파합니다. 감정의 기복을 다스리는 절제력을 보강한다면 타인을 비추는 따뜻한 빛으로 승화될 것입니다."
        elif my_element == "土": part2 += "오상 중 신(신)을 상징하는 대지처럼 묵직하고 신의가 두터우며, 만물을 품는 포용력을 갖추고 있습니다. 매사에 신중하고 원칙을 변함없이 지켜나가는 중용의 덕이 있어 주변 사람들에게 깊은 신뢰를 얻습니다. 변화보다는 안정을 추구하는 경향이 있으나 그 내면에는 무엇이든 수용하고 길러낼 수 있는 무한한 잠재력과 끈기 있는 생명력이 잠재되어 있는 든든한 명조입니다."
        elif my_element == "金": part2 += "오상 중 의(義)를 상징하는 금속의 날카로운 결단력처럼 시비지심이 분명하고 일 처리에 있어 완벽을 기하는 냉철한 성정을 지니고 있습니다. 불필요한 것을 과감히 쳐내는 과단성과 강직한 의리는 귀하를 지도자적 위치로 이끄는 핵심 동력입니다. 시련을 통해 더욱 견고해지는 성취형 인간의 표본이라 할 수 있습니다."
        else: part2 += "오상 중 지(智)를 상징하는 물처럼 유연하고 지혜로우며, 깊은 감수성과 영감을 바탕으로 내면의 통찰을 이뤄내는 명조입니다. 어떤 그릇에도 담길 수 있는 적응력과 보이지 않는 곳에서 흐르며 세상을 적시는 헌신적인 면모가 돋보입니다. 지적 욕구가 강하며 이를 통해 세상을 이롭게 하는 지혜를 전하는 역할을 수행합니다."
