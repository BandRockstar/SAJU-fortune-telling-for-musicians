...
if st.button("🎭 심층 이원 통변 리포트 생성"):
    if name:
        ...
        # 단락 구분 2: 음악적 사주 통변
        st.markdown('<div class="section-header">🎵 음악적 사주 통변 (페르소나 분석)</div>', unsafe_allow_html=True)
        
        # 3. 음악적 분석 (300자 이상)
        music_part = f"**{name}님의 사주 에너지를 음악적 요소로 치환해 보면:** 귀하는 "
        if my_element == "木":
            music_part += "생동감 넘치는 '어쿠스틱 선율'과 닮아 있습니다. 목(木) 기운의 성장 에너지는 현악기의 울림처럼 유기적이며 따뜻한 정서를 자아냅니다. 장르적으로는 포크(Folk)나 실내악 같은 편안하면서도 생명력이 느껴지는 곡조가 귀하의 본질과 공명합니다. 곡의 도입부에서 점진적으로 고조되는 크레셴도(Crescendo)처럼, 시간이 흐를수록 깊이를 더해가는 음악적 서사를 지녔습니다. 이러한 에너지는 사람들의 마음속에 새로운 영감을 심어주는 치유의 선율이 되며, 정형화된 비트보다는 자연스러운 호흡이 느껴지는 리듬에서 본인의 잠재력이 극대화되는 경향이 있습니다."
        elif my_element == "火":
            music_part += "강렬한 에너지를 발산하는 '오케스트라의 금관악기' 또는 '화려한 일렉트로닉 사운드'와 공명합니다. 화(火)의 기운은 무대를 압도하는 솔로 연주자처럼 자신을 표현하는 데 거침이 없으며, 장르적으로는 열정적인 재즈(Jazz)나 웅장한 교향곡의 클라이맥스와 닮아 있습니다. 귀하의 삶은 화려한 화음과 빠른 템포가 어우러진 비르투오소(Virtuoso)의 연주와 같아, 대중에게 강렬한 인상을 남기고 감정을 동화시키는 강력한 흡입력을 발휘합니다. 정적인 배경 속에서도 단숨에 분위기를 반전시키는 음악적 존재감을 지닌 명조라 할 수 있습니다."
        elif my_element == "土":
            music_part += "모든 소리를 안정적으로 받쳐주는 '콘트라베이스' 또는 '대지를 울리는 타악기'의 낮은 공명과 닮아 있습니다. 토(土)의 기운은 음악 전체의 중심을 잡는 베이스 라인처럼 묵직하고 변함없는 신뢰감을 줍니다. 장르적으로는 명상 음악이나 장중한 합창곡처럼 영혼을 안정시키는 깊이 있는 울림이 귀하의 기질과 일치합니다. 화려한 기교보다는 정박자의 정직함과 깊은 배음(Harmonics)을 통해 세상에 안식과 토대를 제공하며, 앙상블에서 결코 빠져서는 안 될 든든한 뿌리와 같은 역할을 수행하는 음악적 에너지를 보유하고 계십니다."
        elif my_element == "金":
            music_part += "명징하고 날카로운 '피아노의 고음역대' 또는 '정교하게 벼려진 금속 악기'의 소리와 닮아 있습니다. 금(金)의 기운은 흐트러짐 없는 완벽한 조율과 정교한 리듬감을 상징하며, 장르적으로는 구조적 미학이 극대화된 바로크 음악이나 현대적인 미니멀리즘 음악과 공명합니다. 귀하의 에너지는 불필요한 잡음이 섞이지 않은 순수한 사인파(Sine Wave)처럼 명확하고 결단력이 있어, 삶의 서사 속에서 군더더기 없는 완벽한 마무리를 지향합니다. 차가운 듯하지만 그 끝에 맺히는 투명한 잔향이 매우 인상적인 음악적 페르소나를 지녔습니다."
        else:
            music_part += "끝없이 깊고 유연한 '플루트의 선율' 또는 '신비로운 앰비언트 사운드'와 공명합니다. 수(水)의 기운은 정해진 형태 없이 흐르는 물처럼 즉흥 연주(Improvisation)에 능하며, 청각적 상상력을 자극하는 몽환적인 화성학과 닮아 있습니다. 장르적으로는 경계가 없는 뉴에이지나 깊은 사유를 유도하는 블루스(Blues)와 일맥상통합니다. 귀하의 삶은 잔잔한 파도에서 폭포수까지 아우르는 넓은 스펙트럼의 다이내믹을 지니고 있어, 듣는 이의 무의식을 어루만지고 감정의 심연으로 안내하는 깊은 영성의 음악을 연주하는 명조입니다."

        music_part += f"\n\n종합적으로 볼 때, {name}님은 사주에 흐르는 오행의 배합을 통해 자신만의 고유한 심포니를 작곡해 나가는 연주자와 같습니다. 강성한 {max_ele}의 기운을 곡의 메인 테마로 삼되, 다른 오행의 기운을 다채로운 변주(Variation)로 활용하신다면 더욱 완성도 높은 인생의 대곡을 완성하실 수 있을 것입니다."
