...
# 2️⃣ 입력 설정 (개인정보 보호 및 시간 모름 옵션 유지)
hour_time_map = {
...
# 3️⃣ 통합 통변 엔진 (장문 + 동적 다양성 결합)
def get_final_report(d_gan, counts, max_elem, name):
    # 일반 성정 데이터 (최소 200자 이상 구성)
    gen_data = {
...
def get_refined_position(d_gan):
    if d_gan in '丙丁':
        ...
        return ("🎸 베이시스트 & 프로듀서", "토(土)의 기운은 조화와 토대입니다. 고음과 저음을 연결하고 모든 악기가 제 소리를 낼 수 있도록 밑바탕을 지탱해 주는 안정적인 사운드의 총지배인입니다.")
...
if submitted:
    if not (y and m and d) or hour_time_map[h_str] == "시간 선택 (또는 모름)":
        ...
        st.markdown(f"""<div class='target-year-card'><h2>🏙️ {target_y}년 운세 흐름</h2><div class='content-text'>{target_y}년은 {d_gan} 일간이 {t_gz}의 흐름을 타고 도약하는 해입니다. 그간 공들인 창작물이 결실을 맺고 사회적 명성을 얻는 긍정적인 운세이므로, 자신감을 가지고 큰 무대에 도전하시길 바랍니다.</div></div>""", unsafe_allow_html=True)
