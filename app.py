...
# 1️⃣ 페이지 설정 및 UI 디자인 (사용자 정의 스타일 완벽 유지)
st.set_page_config(page_title="음악인을 위한 사주통변 Ver 1.5", layout="centered")
st.markdown("""
    <style>
...
st.markdown("<div class='main-title'><h1>🎸 음악인을 위한 사주통변 Ver 1.5</h1></div>", unsafe_allow_html=True)

# 2️⃣ 데이터 엔진: 십성별 5대 악기군 분석 (300자 이상)
analysis_db = {
    '비겁': {
        ...
        'pos_desc': "귀하의 명식에서 가장 빛나는 포지션은 본인의 강력한 자아를 무대 위에서 가감 없이 표출하는 프런트맨입니다. 타인의 간섭 없이 곡의 기획부터 최종 연주까지 본인의 의도대로 끌고 나갈 때 최고의 시너지가 발생합니다.",
        'instrument': "귀하에게 운명적인 악기는 **보컬**과 **기타**입니다. 비겁의 강렬한 에너지는 무대 중앙에서 대중을 압도하는 에너지를 필요로 합니다. 보컬로서는 기교보다 목소리의 톤과 힘으로 청중을 관통하는 스타일이 적합하며, 직선적인 벨팅 창법이 기질과 잘 맞습니다. 기타를 잡는다면 섬세한 연주보다 화려하고 공격적인 리드 기타가 어울립니다. 전체 사운드의 중심을 뚫고 나오는 선명한 존재감을 드러낼 때 예술적 갈증이 해소될 것입니다."
    },
    '식상': {
        'gen': "본인은 천재적인 영감과 표현의 수려함을 타고난 명식으로, 일상의 모든 경험을 예술적 가치로 치환하는 비범한 재능을 지니고 있습니다. 식신과 상관의 발달은 표현력을 극대화하여 타인과의 교감 능력을 높여주며, 창의적인 사고방식을 통해 주변을 밝히는 에너지를 전파합니다.",
        'mus': "창작의 마르지 않는 샘물을 가진 아티스트로서, 멜로디와 가사를 직조하는 감각이 본능적으로 발달해 있습니다. 화려한 연주 테크닉은 물론, 청중의 심리를 파고드는 감성적인 톤 메이킹에 소질을 보입니다. 즉흥적인 영감을 소리로 표현하는 데 능합니다.",
        ...
        'instrument': "다채로운 표현력을 지닌 귀하에게는 **키보드(신디사이저)**와 **기타**가 최적입니다. 식상의 창의성은 무한한 소리의 변화를 추구하므로, 수만 가지 음색을 디자인할 수 있는 키보드를 통해 상상력을 현실화할 때 최고의 희열을 느낍니다. 기타 연주 시에도 이펙터를 적극 활용하여 자신만의 사운드 텍스처를 직조하는 능력이 탁월합니다. 보컬로서는 화려한 애드리브와 넓은 음역대를 활용하는 스타일이 귀하의 기질과 공명합니다."
    },
    '재성': {
        'gen': "본인은 현실을 꿰뚫어 보는 안목과 공간을 장악하는 능력이 남다른 명식으로, 실리적이고 결과 중심적인 사고를 통해 목표를 성취하는 유능한 성품의 소유자입니다. 환경을 조율하는 힘을 상징하며, 본질을 빠르게 파악하여 가장 효율적인 해결책을 찾아냅니다.",
        'mus': "음악을 하나의 거대한 건축물처럼 이해하고 설계하는 프로듀싱 능력이 탁월합니다. 사운드의 전체적인 밸런스와 대중적 성공 가능성을 냉철하게 판단하는 안목을 지녔습니다.",
        'pos_title': "🥁 사운드 디렉터 & 드러머",
        'pos_desc': "전체 사운드를 조율하고 앨범의 가치를 책임지는 프로듀서 역할에 최적입니다. 세션 연주자들의 능력을 적재적소에 배치하는 탁월한 관리 능력을 보여줍니다.",
        'instrument': "음악의 뼈대를 형성하는 귀하에게는 **드럼**과 **베이스**가 운명적입니다. 재성의 현실 장악력은 무대 뒤편에서 전체 앙상블의 리듬을 완벽하게 통제할 때 빛납니다. 드럼은 곡의 구조를 결정짓는 설계도이며, 베이스는 화성과 리듬을 연결하는 가교이기에 본인의 기질과 일치합니다. 화려하게 전면에 나서기보다 전체 사운드가 비지 않도록 채워주는 견고한 플레이에서 깊은 만족감을 느낄 것입니다."
    },
    '관성': {
        'gen': "본인은 명예와 규율을 중요하게 여기는 강직한 성품과 완벽주의를 소유한 명식입니다. 자신에게 엄격한 기준을 적용하는 만큼 타인에게도 정직과 신뢰를 요구하며, 조직 내에서 질서를 유지하는 중추적인 역할을 수행합니다.",
        'mus': "음악적 형식미와 정교한 구조를 추구하는 완벽주의자의 면모를 보여줍니다. 단 한 치의 오차도 허용하지 않는 정밀한 연주와 사운드 디자인을 지향하며, 본질적인 가치를 추구합니다.",
        'pos_title': "🎯 완벽주의 밴드 마스터",
        'pos_desc': "앙상블의 기강을 잡고 사운드의 완성도를 이끄는 리더에 적합합니다. 고도의 집중력을 요하는 소리 편집 및 시스템 설계에 강점이 있습니다.",
        'instrument': "정밀함을 중시하는 귀하에게는 **키보드**와 **베이스**가 가장 적합합니다. 키보드는 화성학적 기초와 정교한 터치가 요구되기에 귀하의 완벽주의를 투영하기 좋고, 베이스 연주 시에도 정확한 박자로 밴드의 품격을 높입니다. 보컬 스타일은 정석적인 발성과 정갈한 창법을 선호합니다. 정해진 약속을 완벽하게 이행하는 모습에서 아티스트로서의 독보적 권위가 세워지게 될 것입니다."
    },
    '인성': {
        'gen': "본인은 깊은 통찰력과 지혜로운 사유를 바탕으로 현상의 본질을 탐구하는 지혜로운 명식입니다. 지식 습득 능력이 뛰어나고 이를 본인만의 철학으로 재해석하여 내놓는 정신적 창조 능력이 탁월합니다.",
        'mus': "음악 속에 심오한 철학적 메시지와 무의식의 영역을 담아내는 능력이 탁월합니다. 단순히 즐거움을 넘어 하나의 세계관을 창조하는 과정을 즐기며, 보이지 않는 정서를 선율로 치환합니다.",
        'pos_title': "🎼 수석 편곡가 & 키보디스트",
        'pos_desc': "곡의 내러티브를 설계하고 세련된 화성으로 사운드를 풍성하게 만드는 편곡 분야에서 독보적입니다. 보이지 않는 감정을 선율로 치환하는 능력을 갖추고 있습니다.",
        'instrument': "깊은 사유를 추구하는 귀하에게는 **기타(어쿠스틱)**와 **키보드(피아노)**가 가장 잘 어울립니다. 풍부한 울림을 가진 악기를 통해 내면세계를 확장하며, 여운이 긴 선율을 직조할 때 영감이 극대화됩니다. 보컬은 이야기를 들려주는 스토리텔링형 보컬이 적합하며, 기술적 화려함보다 감정의 밀도를 중시하여 청중의 무의식을 건드리는 지혜로운 선율을 구사하게 될 것입니다."
    }
}
...
# 4️⃣ 사용자 입력단 (처음 주신 코드 그대로 완벽 복구)
...
if submitted:
    ...
    # 시간 문자열에서 시지 추출 (예: '묘시')
    target_si = h_str.split(" ")[1]
    ba_zi = [lunar.getYearInGanZhi(), lunar.getMonthInGanZhi(), lunar.getDayInGanZhi(), f"??{target_si}"]
    ...
    <div class='target-year-card'>
        ...
        <div class='content-text'>
            <b>{target_y}년은 귀하의 {d_gan} 일간이 세운의 천간 {t_gan}을(를) 만나 '{t_sib}'의 기운이 지배하는 해입니다.</b><br><br>
            이 시기에는 본인이 구축해온 음악적 기반이 외부 환경과 만나 큰 변화를 겪게 됩니다. 
            특히 {t_sib}의 특성에 맞춰 창작의 방향성을 설정한다면 보다 효율적인 예술적 성취를 거둘 수 있습니다.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class='music-card' style='background-color:#FFF5F7;'>
        ...
        <div class='content-text'>
            올해 도래하는 <b>{t_sib}</b>의 기운은 귀하의 창작 세계에 새로운 활력을 불어넣을 것입니다. 
            본인의 음악적 색채 위에 올해의 운이 제공하는 에너지가 더해져, 이전과는 다른 차원의 사운드 메이킹이 가능해집니다. 
            {target_y}년에 기획하는 프로젝트는 장기적으로 커리어에서 중요한 포트폴리오가 될 것입니다.
        </div>
    </div>
    """, unsafe_allow_html=True)
