...
if st.button("🎭 심층 이원 통변 리포트 생성"):
    if name:
        ...
        if birth_time == "모름":
            ...
            # Solar를 통해 시간을 먼저 세팅한 후 Lunar를 가져오는 것이 가장 정확함
            precise_solar = Solar.fromYmdHms(solar_obj.getYear(), solar_obj.getMonth(), solar_obj.getDay(), target_hour, 30, 0)
            eight_char = precise_solar.getLunar().getEightChar()

        gan_ko = {"甲":"갑", "乙":"을", "丙":"병", "丁":"정", "戊":"무", "己":"기", "庚":"경", "辛":"신", "壬":"임", "癸":"계"}
        zi_ko = {"子":"자", "丑":"축", "寅":"인", "卯":"묘", "辰":"진", "巳":"사", "午":"오", "未":"미", "申":"신", "酉":"유", "戌":"술", "亥":"해"}
        gan_elements = {"甲":"木", "乙":"木", "丙":"火", "丁":"火", "戊":"土", "己":"土", "庚":"金", "辛":"金", "壬":"水", "癸":"水"}
        zi_elements = {"寅":"木", "卯":"木", "巳":"火", "午":"火", "申":"金", "酉":"金", "亥":"水", "子":"水", "辰":"土", "戌":"土", "丑":"土", "미":"土"}
        ele_order = ["木", "火", "土", "金", "水"]
        ...
        t_gan, t_zi = format_ganzi(eight_char.getTime())
        ...
        # 1) 오행 기질 상세 분석 (기존 텍스트 그대로)
        p1_text = f"**1) 오행의 균형 및 기질 분석:** {name}님의 사주 구성을 심층 분석한 결과, 전체 8글자 중 {max_ele}의 기운이 {counts[max_ele]}개로 강력하게 작용하고 있습니다. "
        if counts[max_ele] >= 3:
            p1_text += "명리학적으로 특정 오행이 이토록 강성한 명조는 기질이 매우 선명하고 주관이 뚜렷하여 자신만의 독자적인 영역을 구축하는 데 탁월한 능력을 보입니다. 외부의 거센 풍파나 주변의 간섭에도 불구하고 자신의 신념과 원칙을 관철해 나가는 강인한 추진력을 소유하고 계십니다. 본인의 에너지가 집중된 만큼 전문 분야에서의 성취는 보장되어 있습니다."
        else:
            p1_text += "오행의 분포가 비교적 고르게 분포되어 성품이 원만하고 매사에 균형 감각이 대단히 뛰어난 명조입니다. 객관적인 시각으로 본질을 바라볼 줄 알며 주변 사람들과 조화롭게 융화되는 유연한 처세술이 강점입니다."
        st.info(p1_text)

        # 2) 본신 성정 (리더님의 火 기질 기준 상세 텍스트 예시 포함)
        p2_text = f"**2) 본신의 성정과 심리적 특징:** 본신을 상징하는 일간 {my_day_gan}은 {my_element}의 근본 성질을 가집니다. "
        if my_element == "火":
            p2_text += "오상 중 예(禮)를 상징하는 불꽃처럼 열정적이고 화려하며, 자신을 드러내어 소통하는 표현력이 강력합니다. 소리와 빛으로 타인의 가슴에 감동을 새기는 예술적 본능이 발달한 명조입니다."
        # ... (이하 다른 오행 조건문 생략, 구조는 동일하게 작동)
        st.info(p2_text)

        # 3) 음악적 통변 (기존 내용 유지)
        st.markdown('<div class="section-header">🎵 음악적 사주 통변 (밴드 페르소나 분석)</div>', unsafe_allow_html=True)
        if my_element == "火":
            st.info("**1) 추천 음악 포지션:** 관객의 시선을 사로잡는 보컬이나 화려한 리드 기타가 최적입니다.")
            st.info("**2) 음악적 성향:** 명확하고 다이내믹한 사운드를 지향하며 청중의 심박수를 조절하는 연출에 능합니다.")
            st.info("**3) 밴드 내 위치:** 팀의 에너지를 책임지는 프론트맨이자 정신적 지주 역할을 수행합니다.")
        ...
        if len(today_ganzi) >= 2:
            ...
            t_music_db = {
                0: {"title": "솔로(Solo) & 주관", "desc": "자아의 에너지가 충만합니다. 본인의 직관을 믿고 톤을 잡아보세요."},
                1: {"title": "즉흥(Improvisation) & 창의", "desc": "표현의 통로가 열립니다. 손 가는 대로 연주해 보세요."},
                2: {"title": "앙상블(Ensemble) & 결실", "desc": "조화가 뛰어납니다. 합주나 녹음에서 성과가 납니다."},
                3: {"title": "그루브(Groove) & 절제", "desc": "정교한 컨트롤이 필요합니다. 기본기에 충실하세요."},
                4: {"title": "리조넌스(Resonance) & 심화", "desc": "내면의 깊이를 더합니다. 새로운 사운드 연구에 유리합니다."}
            }
...
---
**💡 리더의 조언:**
오늘의 기운은 '{n_ele}'입니다. 본연의 기질인 '{my_element}'와 만나 새로운 흐름을 만드니, 기술적 완벽함보다 오늘만 느낄 수 있는 그루브를 악기에 담아보세요.
""")
    else:
